        ...
        <div class="col-12">
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <div class="row">
                            <form action="/admin/book/index" method="get" class="form-inline" enctype="multipart/form-data">
                                ...
                                <table id="example2" class="table table-bordered table-hover dataTable dtr-inline"
                                    ...
                                    <tr>
                                        ...
                                        <th class="sorting" tabindex="0" aria-controls="example2" rowspan="1"
                                            ...
                                            {{trans('default.book_control.book_auther') ?? '作者'}}
                                        </th>
                                        <th class="sorting" tabindex="0" aria-controls="example2" rowspan="1"
                                            colspan="1"
                                        ...
                                        <th class="sorting" tabindex="0" aria-controls="example2" rowspan="1"
                                            colspan="1"
                                            aria-label="CSS grade: activate to sort column ascending">{{trans('default.book_control.book_word_num') ?? '字數'}}
                                        </th>
                                        <th class="sorting" tabindex="0" aria-controls="example2" rowspan="1"
                                            colspan="1"
                                        ...
                                        <th class="sorting" tabindex="0" aria-controls="example2" rowspan="1"
                                            colspan="1"
                                            aria-label="CSS grade: activate to sort column ascending">{{trans('default.book_control.book_latest_chapter') ?? '最新章節'}}
                                        </th>
                                        <th class="sorting" tabindex="0" aria-controls="example2" rowspan="1"
                                            colspan="1"
                                            aria-label="CSS grade: activate to sort column ascending">{{trans('default.book_control.book_update_time') ?? '更新時間'}}
                                        </th>
                                        <th class="sorting" tabindex="0" aria-controls="example2" rowspan="1"
                                            colspan="1"
                                    ...
                                    @foreach($datas as $book)
                                        <tr class="odd">
                                            ...
                                            <td>{{ $book->latest_chapter }}</td>
                                            <td>{{ $book->update_time }}</td>
                                            <td>
                                                @if(authPermission('book-recommend'))
                                                    <!-- <a onclick="recommend()" class="btn btn-primary">{{trans('default.recommend') ?? '推薦'}}</a> -->

                                                    <form action="/admin/book/recommend" method="post">
                                                        <input type="hidden" name="book_type_id" value="{{$bookTypeId}}" >
                                                        <input type="hidden" name="book_name" value="{{$bookName}}" >
                                                        <input type="hidden" name="book_id" value="{{$book->book_id}}" >
                                                        <input type="hidden" name="page" value="{{$page}}" >
                                                        @if(! $book->book_recommend_id)
                                                        <input type="submit"  class="btn btn-primary" value="{{trans('default.recommend') ?? '推薦'}}">
                                                        @else
                                                        <input type="submit"  class="btn btn-primary" disabled="true" value="已推薦">
                                                        @endif
                                                    </form>
                                                @endif

                                                @if(authPermission('book-delete'))
                                                    <form action="/admin/book/delete" method="post" _method="delete">
                                    ...
                                    <tr>
                                        ...
                                        <th rowspan="1" colspan="1">{{trans('default.book_control.book_auther') ?? '作者'}}</th>
                                        <th rowspan="1" colspan="1">{{trans('default.book_control.book_status') ?? '狀態'}}</th>
                                        <th rowspan="1" colspan="1">{{trans('default.book_control.book_word_num') ?? '字數'}}</th>
                                        <th rowspan="1" colspan="1">{{trans('default.book_control.book_img') ?? '封面'}}</th>
                                        <th rowspan="1" colspan="1">{{trans('default.book_control.book_latest_chapter') ?? '最新章節'}}</th>
                                        <th rowspan="1" colspan="1">{{trans('default.book_control.book_update_time') ?? '更新時間'}}</th>
