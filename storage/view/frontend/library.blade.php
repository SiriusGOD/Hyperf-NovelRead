        ...
        <div class="container">
            ...
            <div class="class">
                <ul>
                    ...
                    <li>
                        <a href="{{$library_url}}?typeId={{$list->book_type_id}}">{{$list->type_name}}</a>
                    </li>
                    @endforeach
                </ul>
            </div>
            ...
            <div class="pages">
                <div class="pagelink" id="pagelink">
                    ...
                    <a href="{{$library_url}}?typeId={{$type_id}}&page=1" class="first">1</a>
                    <a href="{{$library_url}}?typeId={{$type_id}}&page={{$page - 1}}" class="pgroup">&lt;</a>
                    @if($total_page_num <= 5)
                        @for($i = 1; $i <= $total_page_num; $i++)
                            ...
                            @elseif($page == $i)
                                <strong>{{$page}}</strong>
                            @else
                                <a href="{{$library_url}}?typeId={{$type_id}}&page={{$i}}">{{$i}}</a>
                            @endif
                        @endfor
                    @else
                        @if(($total_page_num - $page) >= 3 && $page > 2)
                            @for($i = -2; $i <= 2; $i++)
                                ...
                                @else
                                    <a href="{{$library_url}}?typeId={{$type_id}}&page={{$page + $i}}">{{$page + $i}}</a>
                                @endif
                            @endfor
                    ...
                    @if($page + $i > $total_page_num)
                        <a href="{{$library_url}}?typeId={{$type_id}}&page={{$page}}" class="next">&gt;</a>  
                    @else
                        <a href="{{$library_url}}?typeId={{$type_id}}&page={{$page + 1}}" class="next">&gt;</a> 
                    @endif

                    <a href="{{$library_url}}?typeId={{$type_id}}&page={{$total_page_num}}" class="last">{{$total_page_num}}</a>
                </div>
            </div>
            <div class="clear"></div>
        </div>
